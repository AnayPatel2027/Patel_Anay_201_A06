...
names(points_per_game) <- c("MJ", "LJ", "KB", "KA", "SO")
...
rounded_points_per_game <- sapply(points_per_game, function(x) round(x,2))
...
mat_rand <- apply(mat_rand, MARGIN = 1:2, function(x) signif(x, 1))
mat_rand
...
temps <- max_temps[,3:14]
temps <- lapply(temps, as.numeric)
av_temp <- list()


for(i in 1:length(temps)){
  av_temp <- append(av_temp, mean(temps[[i]]))
}


names(av_temp) <- c("JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC")
...
for(i in 3:ncol(max_temps)){
  av_temps <- append(av_temps, mean(unlist(max_temps[,i])))
}


names(av_temps) <-  c("JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC")
...
for(i in 1:nrow(CO_df)){
  med_temp <- append(med_temp, median(as.numeric(CO_df[i, 3:14])))
}
